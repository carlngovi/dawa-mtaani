...
<div class="space-y-6">
    ...
    <a href="/super/t0-approvals"
       class="bg-gray-800 rounded-xl border border-gray-700 p-5 block hover:shadow-md hover:border-gray-600 transition-all">
        <p class="text-xs text-gray-400">Pending Approvals</p>
        <p class="text-3xl font-bold text-{{ $pendingApprovals > 0 ? 'amber-400' : 'white' }} mt-1">
            {{ $pendingApprovals }}
        </p>
        <p class="text-xs text-green-400 mt-2 font-medium">View at /super/t0-approvals →</p>
    </a>

    {{-- Request form --}}
    <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-700 bg-gray-900/50">
            <h3 class="text-sm font-semibold text-gray-300">Submit Write Request</h3>
        </div>
        <form method="POST" action="/api/v1/tech/write-request" class="p-5 space-y-4">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-400 mb-1">Operation Type</label>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-400 mb-1">Affected Tables</label>
