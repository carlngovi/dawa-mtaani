...
@section('title', 'Submissions — County Coordinator')
@section('content')
<div class="space-y-6">
    <h1 class="text-2xl font-bold text-white">County Submissions</h1>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-700">
                ...
                <tr class="hover:bg-gray-700/50"><td class="px-4 py-3 text-white">{{ $sub->pharmacy }}</td><td class="px-4 py-3 text-gray-400">{{ $sub->ward }}</td><td class="px-4 py-3 text-gray-400">{{ $sub->spotter?->name ?? '—' }}</td><td class="px-4 py-3 text-gray-400">{{ $sub->visit_date }}</td>
                    ...
                    <td class="px-4 py-3 text-right"><a href="{{ route('county.submissions.show', $sub) }}" class="text-yellow-400 text-xs hover:underline">View</a></td>
                </tr>
                @empty
