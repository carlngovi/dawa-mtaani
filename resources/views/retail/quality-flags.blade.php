...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-6">
            <h3 class="text-sm font-semibold text-gray-200 mb-4">Submit a Quality Report</h3>
            <form method="POST" action="/api/v1/quality-flags" class="space-y-4"
                ...
                <div>
                    ...
                    <select name="flag_type" required class="h-10 w-full rounded-lg border border-gray-600 bg-gray-800 px-3 text-sm text-white focus:border-yellow-400 focus:outline-none focus:ring-1 focus:ring-yellow-400">
                        <option value="">— Select —</option>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" placeholder="Describe the issue..." class="w-full rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white placeholder-gray-500 focus:border-yellow-400 focus:outline-none focus:ring-1 focus:ring-yellow-400 resize-none"></textarea>
                </div>
        ...
        <div class="lg:col-span-2 bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-700"><h3 class="text-sm font-semibold text-gray-200">My Submitted Reports</h3></div>
            <div class="overflow-x-auto"><table class="w-full text-sm min-w-[560px]">
                <thead class="bg-gray-900/50 text-xs text-gray-400 uppercase"><tr><th class="px-4 py-3 text-left">Product</th><th class="px-4 py-3 text-left">Type</th><th class="px-4 py-3 text-left">Status</th><th class="px-4 py-3 text-left">Date</th></tr></thead>
                <tbody class="divide-y divide-gray-700">
                    @forelse($myFlags as $flag)
                    <tr class="hover:bg-gray-900/50">
                        <td class="px-4 py-3"><p class="font-medium text-gray-200">{{ $flag->generic_name ?? 'Unknown' }}</p></td>
                        <td class="px-4 py-3"><span class="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-red-900/30 text-red-400">{{ str_replace('_', ' ', $flag->flag_type) }}</span></td>
                        <td class="px-4 py-3"><span class="inline-flex px-2 py-0.5 rounded text-xs font-medium {{ match($flag->status) { 'OPEN'=>'bg-amber-900/30 text-amber-400','UNDER_REVIEW'=>'bg-blue-900/30 text-blue-400','CONFIRMED'=>'bg-red-900/30 text-red-400',default=>'bg-gray-700 text-gray-400' } }}">{{ str_replace('_', ' ', $flag->status) }}</span></td>
            ...
            <div class="px-5 py-3 border-t border-gray-700">{{ $myFlags->links() }}</div>
        </div>
    </div>
</div>
@endsection
