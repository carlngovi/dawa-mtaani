...
<div class="space-y-6" x-data="catalogue()" x-init="init()">
    ...
    <form method="GET" class="space-y-3">
        <div class="relative">
            ...
            <button type="submit" class="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-yellow-400 text-white rounded-lg text-xs font-medium hover:bg-yellow-500 transition-colors">
                Search
            </button>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
            <a href="/retail/catalogue"
               class="px-3 py-1.5 rounded-full text-xs font-medium transition-colors {{ !request('category') ? 'bg-yellow-400 text-white' : 'bg-gray-800 border border-gray-600 text-gray-300 hover:border-gray-700' }}">
                All
            </a>
            @foreach($categories as $cat)
            <a href="/retail/catalogue?category={{ urlencode($cat) }}{{ request('search') ? '&search=' . urlencode(request('search')) : '' }}"
               class="px-3 py-1.5 rounded-full text-xs font-medium transition-colors {{ request('category') === $cat ? 'bg-yellow-400 text-white' : 'bg-gray-800 border border-gray-600 text-gray-300 hover:border-gray-700' }}">
                {{ $cat }}
            </a>
            @endforeach
        </div>
        ...
        <div class="rounded-xl bg-gray-800 border border-gray-700 p-4 flex flex-col gap-3 hover:border-gray-600 transition-colors">
            ...
            <div class="flex items-start justify-between gap-2">
                <div class="flex-1 min-w-0">
                    ...
                    @endif
                    <p class="text-gray-300 text-xs font-mono">{{ $product->sku_code }}</p>
                </div>
                <button onclick="toggleFavourite({{ $product->product_id }})"
                        class="text-lg flex-shrink-0 {{ in_array($product->product_id, $favouriteIds) ? 'text-yellow-400' : 'text-gray-300' }} hover:text-yellow-400 transition-colors">
                    ★
                </button>
            </div>
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-blue-900/30 text-blue-400 border border-gray-700">{{ $product->therapeutic_category }}</span>
            </div>
            ...
            <div x-show="!inCart({{ $product->price_list_id }})">
                <button @click="addToCart({{ $product->product_id }}, {{ $product->price_list_id }}, '{{ addslashes($product->generic_name) }}', {{ $product->unit_price }})"
                        class="w-full py-2 bg-yellow-400 hover:bg-yellow-500 text-white text-sm font-medium rounded-lg transition-colors">
                    + Add to Order
                </button>
            </div>
    ...
    <a href="/retail/basket" x-show="cartCount > 0" x-cloak
       class="fixed bottom-6 right-6 z-40 flex items-center gap-2 rounded-full bg-yellow-400 px-5 py-3 text-white shadow-lg hover:bg-yellow-500 transition-colors font-medium text-sm">
