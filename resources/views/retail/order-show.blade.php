...
<div class="space-y-6" x-data="{ showDisputeModal: false }">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-2 text-sm">
            ...
            <div class="flex justify-between"><span class="text-gray-400">Credit</span>
                <span class="text-blue-600">{{ $currency['symbol'] }} {{ number_format($order->credit_amount, $currency['decimal_places']) }}</span>
            </div>
            @endif
        ...
        @if($courier)
        <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-2 text-sm">
            <h3 class="font-semibold text-gray-700">Dispatch</h3>
        ...
        {{-- Dispute status --}}
        @if($dispute)
        <div class="bg-white rounded-xl border border-{{ $dispute->status === 'OPEN' ? 'amber' : ($dispute->status === 'RESOLVED' ? 'green' : 'gray') }}-200 p-5 space-y-2 text-sm">
            <h3 class="font-semibold text-gray-700">Dispute</h3>
            ...
            <div class="flex justify-between"><span class="text-gray-400">SLA Deadline</span>
                <span class="{{ $dispute->sla_breached ? 'text-red-600 font-semibold' : '' }}">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr>
                    ...
                    <td class="px-5 py-3 text-xs text-gray-500 hidden md:table-cell">{{ $line->supplier_name }}</td>
            ...
            <tfoot class="border-t border-gray-200 bg-gray-50">
                <tr>
                    <td colspan="5" class="px-5 py-3 text-right font-semibold text-gray-700">Total</td>
                    <td class="px-5 py-3 text-right font-bold text-gray-900">{{ $currency['symbol'] }} {{ number_format($order->total_amount, $currency['decimal_places']) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
    ...
    <div x-show="showDisputeModal" x-cloak
        ...
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" @click.outside="showDisputeModal = false">
            <h3 class="text-base font-semibold text-gray-900">Raise Dispute</h3>
            <p class="text-sm text-gray-500 mt-1">Describe the issue with your delivered order.</p>
            <form method="POST" action="/retail/orders/{{ $order->ulid }}/dispute" class="mt-4 space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Dispute Type</label>
                    <select name="reason" required
                            class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Description (min 20 characters)</label>
                ...
                <div class="flex gap-3">
                    ...
                    <button type="button" @click="showDisputeModal = false" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Cancel</button>
                </div>
            </form>
        </div>
    </div>
