...
<div class="space-y-6">
    ...
    @if(session('success'))
    <div class="rounded-lg bg-green-900/20 border border-gray-700 px-4 py-3 text-sm text-green-300">
        {{ session('success') }}
    </div>
    @endif
        ...
        <form method="POST" action="/api/v1/pos/dispense" class="p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="flex items-end">
                    <button type="submit"
                            class="w-full px-4 py-2 bg-yellow-400 hover:bg-yellow-500 text-white text-sm font-medium rounded-lg transition-colors">
                        Record
                    </button>
                </div>
            </div>
