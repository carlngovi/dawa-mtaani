...
@section('title', 'Duplicate Reviews — Sales Rep')
@section('content')
<div class="space-y-6">
    <h1 class="text-2xl font-bold text-white">Tier 1 Duplicate Reviews</h1>
    <p class="text-sm text-gray-400">Review submissions flagged as potential duplicates of existing pharmacies</p>
    @forelse($reviews as $review)
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-lg p-5">
            ...
            <div class="flex gap-3 mt-4 pt-4 border-t border-gray-700">
                <form method="POST" action="{{ route('sales.duplicates.decide', $review) }}">@csrf<input type="hidden" name="decision" value="confirmed_duplicate"><button class="px-4 py-2 bg-red-400/20 text-red-400 rounded-lg text-sm font-medium hover:bg-red-400/30">Confirm Duplicate</button></form>
                <form method="POST" action="{{ route('sales.duplicates.decide', $review) }}">@csrf<input type="hidden" name="decision" value="not_duplicate"><button class="px-4 py-2 bg-green-400/20 text-green-400 rounded-lg text-sm font-medium hover:bg-green-400/30">Not a Duplicate → CC</button></form>
            </div>
        </div>
    @empty
        <div class="bg-gray-800 border border-gray-700 rounded-lg p-8 text-center text-gray-500">No pending duplicate reviews.</div>
    @endforelse
