...
@section('title', 'Follow-ups — Sales Rep')
@section('content')
<div class="space-y-6">
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
        <table class="w-full text-sm">
            <thead><tr class="border-b border-gray-700 text-gray-400 text-xs uppercase tracking-widest">
                <th class="px-4 py-3 text-left">Pharmacy</th><th class="px-4 py-3 text-left">Ward</th><th class="px-4 py-3 text-left">Due Date</th><th class="px-4 py-3 text-left">Next Step</th><th class="px-4 py-3 text-center">Status</th>
            </tr></thead>
            <tbody class="divide-y divide-gray-700">
                @forelse($followups as $f)
                <tr><td class="px-4 py-3 text-white">{{ $f->submission?->pharmacy ?? '—' }}</td><td class="px-4 py-3 text-gray-400">{{ $f->submission?->ward ?? '—' }}</td><td class="px-4 py-3 {{ ($f->status->value ?? $f->status) === 'overdue' ? 'text-red-400' : 'text-gray-400' }}">{{ $f->follow_up_date?->format('Y-m-d') ?? '—' }}</td><td class="px-4 py-3 text-gray-400">{{ str_replace('_',' ',$f->next_step ?? '—') }}</td>
