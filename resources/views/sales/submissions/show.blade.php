...
@section('title', $submission->pharmacy . ' — Sales Rep')
@section('content')
<div class="space-y-6 max-w-4xl">
    <div class="flex items-center gap-3">
        <a href="{{ route('sales.submissions.index') }}" class="text-gray-400 hover:text-white"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg></a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-2">
            ...
            <p class="text-gray-400 text-sm">Present: {{ $submission->owner_present ? 'Yes' : 'No' }}</p>
        </div>
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-2">
            ...
            <p class="text-gray-300 text-sm">Next Step: {{ str_replace('_', ' ', $submission->next_step ?? '—') }}</p>
        </div>
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-2">
            ...
            @if($submission->notes)<p class="text-gray-500 text-xs mt-2">Notes: {{ $submission->notes }}</p>@endif
        </div>
    </div>
</div>
@endsection
