...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Credit Positions</h1>
            <p class="text-sm text-gray-500 mt-1">All facility credit accounts</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl border border-green-200 p-5">
            ...
            <p class="text-3xl font-bold text-green-700 mt-1">{{ $summary->active_count ?? 0 }}</p>
        </div>
        <div class="bg-white rounded-xl border border-{{ ($summary->pending_count ?? 0) > 0 ? 'amber' : 'gray' }}-200 p-5">
            <p class="text-xs text-gray-400">Pending Assessment</p>
            <p class="text-3xl font-bold text-{{ ($summary->pending_count ?? 0) > 0 ? 'amber-600' : 'gray-900' }} mt-1">
                {{ $summary->pending_count ?? 0 }}
            </p>
        </div>
        <div class="bg-white rounded-xl border border-{{ ($summary->suspended_count ?? 0) > 0 ? 'red' : 'gray' }}-200 p-5">
            <p class="text-xs text-gray-400">Suspended</p>
            <p class="text-3xl font-bold text-{{ ($summary->suspended_count ?? 0) > 0 ? 'red-600' : 'gray-900' }} mt-1">
            ...
            <table class="w-full text-sm min-w-[800px]">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-5 py-3 font-medium text-gray-800">{{ $account->facility_name }}</td>
                        <td class="px-5 py-3 text-gray-500 hidden md:table-cell">{{ $account->county }}</td>
                        ...
                        <td class="px-5 py-3 text-xs text-gray-600">{{ $account->tranche_name }}</td>
