...
<div class="w-full max-w-md space-y-4"
     x-data="paymentPoller('{{ $order->ulid }}', '/store/orders/{{ $order->ulid }}/payment-status')"
     x-init="init()">
    ...
    <div class="rounded-xl bg-gray-800 border border-gray-700 p-8 text-center space-y-5">
        ...
        <div class="bg-gray-900 rounded-xl px-4 py-3 border border-gray-700">
            ...
            <p class="text-yellow-400 font-mono text-sm font-semibold mt-0.5">{{ $currency['symbol'] }} {{ number_format($order->total_amount, $currency['decimal_places']) }}</p>
        </div>
        ...
        <div class="space-y-2">
            ...
            <a x-show="paid" x-cloak :href="redirectUrl || '/store/orders'" class="block w-full py-3 bg-yellow-400 hover:bg-yellow-500 text-white font-semibold rounded-xl text-center transition-colors">View My Order →</a>
            <a x-show="failed" x-cloak href="/store/basket" class="block w-full py-3 bg-yellow-400 hover:bg-yellow-500 text-white font-semibold rounded-xl text-center transition-colors">Try Again →</a>
            ...
            <a href="/store/orders" class="block text-xs text-gray-300 hover:text-gray-400 text-center transition-colors">View My Orders</a>
        </div>
    </div>
