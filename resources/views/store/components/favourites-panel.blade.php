{{-- Favourites Panel --}}
<div x-data="{ favourites: [] }" x-init="fetchFavourites()" class="rounded-xl border bg-white p-6 shadow-sm">
    <h3 class="text-lg font-semibold text-gray-900 mb-4">Your Favourites</h3>
        ...
        <template x-for="fav in favourites" :key="fav.product_ulid">
            <div class="flex flex-col rounded-lg border p-3 hover:shadow-md transition">
                <div class="flex items-start justify-between">
                    <p class="text-sm font-medium text-gray-900 line-clamp-2" x-text="fav.generic_name"></p>
                ...
                <p class="mt-1 text-xs text-gray-500" x-text="fav.brand_name"></p>
                <p class="mt-auto pt-2 text-sm font-semibold text-indigo-600" x-text="fav.unit_price"></p>
                <button @click="addToBasket(fav)" class="mt-2 rounded-md bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-indigo-700 transition">
                    Add to Basket
                </button>
            </div>
        </template>
