...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-5 space-y-3">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="font-mono text-gray-300">{{ $facility->ppb_licence_number }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-400">Type</span>
                    <span class="text-gray-300">{{ $facility->ppb_facility_type }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-400">PPB Status</span>
                    <span class="text-gray-300">{{ $facility->ppb_licence_status }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-400">Membership</span>
                    <span class="text-gray-300">{{ $facility->network_membership }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-400">County</span>
                    <span class="text-gray-300">{{ $facility->county }}, {{ $facility->ward }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-400">Phone</span>
                    <span class="text-gray-300">{{ $facility->phone }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-400">GPS</span>
                    @if($facility->latitude)
                        <span class="text-green-400 text-xs">{{ $facility->latitude }}, {{ $facility->longitude }}</span>
                    @else
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-5">
            <h3 class="text-sm font-semibold text-gray-300 mb-3">Authorised Placers</h3>
            @forelse($authorisedPlacers as $placer)
                <div class="text-sm py-2 border-b border-gray-700 last:border-0">
                    <p class="font-medium text-gray-200">{{ $placer->name }}</p>
        ...
        {{-- Active flags --}}
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-5">
            <h3 class="text-sm font-semibold text-gray-300 mb-3">Active Flags</h3>
            ...
            <tbody class="divide-y divide-gray-700">
                @forelse($recentOrders as $order)
                    <tr class="hover:bg-gray-900">
                        ...
                        <td class="px-5 py-3 text-gray-400">{{ $order->order_type }}</td>
                        <td class="px-5 py-3 text-right text-gray-200">{{ number_format($order->total_amount, 2) }}</td>
