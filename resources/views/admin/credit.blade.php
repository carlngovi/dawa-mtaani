...
<div class="p-6 space-y-8">
  ...
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-xl font-semibold text-white">Credit Engine Configuration</h1>
      <p class="text-sm text-gray-400 mt-1">Tranches · Tiers · Progression Rules · Changelog — zero hardcoded values</p>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
    <div class="rounded-xl bg-gray-800 border border-gray-700 p-4">
      <p class="text-xs text-gray-400 uppercase tracking-wide">Active accounts</p>
      <p class="text-2xl font-semibold text-white mt-1">{{ $totalActive }}</p>
    </div>
    <div class="rounded-xl bg-gray-800 border border-gray-700 p-4">
      <p class="text-xs text-gray-400 uppercase tracking-wide">Suspended</p>
      <p class="text-2xl font-semibold text-red-400 mt-1">{{ $totalSuspended }}</p>
    </div>
    <div class="rounded-xl bg-gray-800 border border-gray-700 p-4">
      <p class="text-xs text-gray-400 uppercase tracking-wide">Pending assessment</p>
      <p class="text-2xl font-semibold text-amber-400 mt-1">{{ $totalPending }}</p>
    </div>
  </div>
  ...
  <div class="rounded-xl bg-gray-800 border border-gray-700 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-700 flex items-center justify-between">
      <h2 class="font-semibold text-white">Tranches</h2>
      <button onclick="document.getElementById('new-tranche-form').classList.toggle('hidden')" class="text-sm text-yellow-400 hover:underline">+ Add tranche</button>
    </div>

    <div id="new-tranche-form" class="hidden px-5 py-4 bg-gray-900/50 border-b border-gray-700">
      <form method="POST" action="{{ route('admin.credit.tranches.store') }}" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        @csrf
        <div><label class="block text-xs text-gray-400 mb-1">Name</label><input type="text" name="name" required class="w-full text-sm rounded-lg border border-gray-600 bg-gray-800 px-3 py-2"></div>
        <div><label class="block text-xs text-gray-400 mb-1">Approval pathway</label><select name="approval_pathway" class="w-full text-sm rounded-lg border border-gray-600 bg-gray-800 px-3 py-2"><option value="AUTOMATIC">Automatic</option><option value="ASSESSED">Assessed</option></select></div>
        <div><label class="block text-xs text-gray-400 mb-1">Entry amount ({{ $currency['symbol'] }})</label><input type="number" step="0.01" name="entry_amount" required class="w-full text-sm rounded-lg border border-gray-600 bg-gray-800 px-3 py-2"></div>
        <div><label class="block text-xs text-gray-400 mb-1">Ceiling amount — blank for fixed</label><input type="number" step="0.01" name="ceiling_amount" class="w-full text-sm rounded-lg border border-gray-600 bg-gray-800 px-3 py-2"></div>
        <div><label class="block text-xs text-gray-400 mb-1">Effective from</label><input type="date" name="effective_from" required class="w-full text-sm rounded-lg border border-gray-600 bg-gray-800 px-3 py-2"></div>
    ...
    <div class="px-5 py-5 border-b border-gray-700 last:border-0" x-data="{ open: false }">
      <div class="flex items-start justify-between">
        <div class="flex items-center gap-3">
          <span @click="open = !open" class="cursor-pointer text-gray-400 hover:text-gray-400 text-xs select-none"><span x-show="!open">▶</span><span x-show="open">▼</span></span>
          <div>
            ...
            <p class="text-xs text-gray-400 mt-0.5">Entry: {{ $currency['symbol'] }} {{ number_format($tranche->entry_amount, $currency['decimal_places']) }}@if($tranche->ceiling_amount) · Ceiling: {{ $currency['symbol'] }} {{ number_format($tranche->ceiling_amount, $currency['decimal_places']) }}@endif</p>
          </div>
        </div>
        <form method="POST" action="{{ route('admin.credit.tranches.toggle', $tranche) }}">@csrf @method('PATCH')<button type="submit" class="text-xs text-gray-400 hover:text-gray-400">{{ $tranche->is_active ? 'Deactivate' : 'Activate' }}</button></form>
      </div>
      ...
      <div x-show="open" class="mt-4 ml-6 space-y-4">
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide">Parties</p>
            @php $riskSum = $tranche->activeParties->sum('risk_percentage'); @endphp
            <span class="text-xs {{ abs($riskSum - 100) < 0.01 ? 'text-green-400' : 'text-red-500' }}">Risk sum: {{ $riskSum }}% {{ abs($riskSum - 100) < 0.01 ? '✓' : '⚠ must = 100%' }}</span>
          </div>
          @if($tranche->activeParties->isNotEmpty())
          <div class="overflow-x-auto"><table class="min-w-full text-xs"><thead><tr class="text-gray-400 border-b border-gray-700"><th class="text-left pb-1 pr-4">Party</th><th class="text-left pb-1 pr-4">Type</th><th class="text-right pb-1 pr-4">Risk %</th><th class="text-right pb-1 pr-4">Return %</th><th class="text-left pb-1">Binding</th></tr></thead><tbody>
            @foreach($tranche->activeParties as $party)
            <tr class="border-b border-gray-50"><td class="py-1.5 pr-4 text-gray-200">{{ $party->party_name }}</td><td class="py-1.5 pr-4 text-gray-400">{{ $party->party_type }}</td><td class="py-1.5 pr-4 text-right font-mono text-gray-200">{{ $party->risk_percentage }}%</td><td class="py-1.5 pr-4 text-right font-mono text-gray-200">{{ $party->return_percentage }}%</td><td class="py-1.5 text-gray-400 font-mono text-xs">{{ $party->banking_party_binding ?? '—' }}</td></tr>
            @endforeach
          </tbody></table></div>
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide">Tiers</p>
            @php $allocSum = $tranche->activeTiers->sum('allocation_pct'); @endphp
            <span class="text-xs {{ abs($allocSum - 100) < 0.01 || $allocSum == 0 ? 'text-green-400' : 'text-red-500' }}">Allocation sum: {{ $allocSum }}% {{ abs($allocSum - 100) < 0.01 ? '✓' : ($allocSum == 0 ? '(add tiers)' : '⚠ must = 100%') }}</span>
          </div>
          @if($tranche->activeTiers->isNotEmpty())
          <div class="space-y-1">
            @foreach($tranche->activeTiers as $tier)
            <div class="flex items-center justify-between text-xs bg-gray-900/50/40 rounded px-3 py-2">
              <div><span class="font-medium text-gray-200">{{ $tier->name }}</span><span class="ml-2 text-gray-400">Unlock {{ $tier->unlock_threshold_pct }}%</span><span class="ml-2 text-gray-400">Alloc {{ $tier->allocation_pct }}%</span>@if($tier->approval_required)<span class="ml-2 text-amber-400">Approval req</span>@endif</div>
  ...
  <div class="rounded-xl bg-gray-800 border border-gray-700 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-700">
      <h2 class="font-semibold text-white">Progression Rules</h2>
      <p class="text-xs text-gray-400 mt-0.5">How credit grows based on repayment speed.</p>
    </div>
    ...
    <div class="px-5 py-4 bg-gray-900/50 border-t border-gray-700">
      <form method="POST" action="{{ route('admin.credit.progression.store') }}" class="grid grid-cols-2 sm:grid-cols-5 gap-2 items-end">@csrf
        <div><label class="block text-xs text-gray-400 mb-1">Label</label><input type="text" name="label" required placeholder="e.g. On-time" class="w-full text-xs rounded border border-gray-600 bg-gray-800 px-2 py-1.5"></div>
  ...
  <div class="rounded-xl bg-gray-800 border border-gray-700 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-700">
      <h2 class="font-semibold text-white">Config Changelog</h2>
      <p class="text-xs text-gray-400 mt-0.5">Immutable — every field-level change recorded automatically.</p>
    </div>
    ...
    <div class="overflow-x-auto"><table class="min-w-full text-xs">
      <thead><tr class="border-b border-gray-700 text-gray-400"><th class="text-left px-5 py-2">When</th><th class="text-left px-4 py-2">By</th><th class="text-left px-4 py-2">Model</th><th class="text-left px-4 py-2">Field</th><th class="text-left px-4 py-2">Before</th><th class="text-left px-4 py-2">After</th></tr></thead>
      <tbody class="divide-y divide-gray-50">
        @foreach($changelog as $entry)
        <tr><td class="px-5 py-2 text-gray-400 whitespace-nowrap">{{ \Carbon\Carbon::parse($entry->changed_at)->timezone('Africa/Nairobi')->format('d M Y H:i') }}</td><td class="px-4 py-2 text-gray-300">{{ $entry->changedByUser?->name ?? 'System' }}</td><td class="px-4 py-2 text-gray-400 font-mono">{{ class_basename($entry->model_type) }}#{{ $entry->model_id }}</td><td class="px-4 py-2 text-gray-300 font-mono">{{ $entry->field_name }}</td><td class="px-4 py-2 text-red-500 font-mono max-w-xs truncate">{{ $entry->value_before ?? '—' }}</td><td class="px-4 py-2 text-green-400 font-mono max-w-xs truncate">{{ $entry->value_after ?? '—' }}</td></tr>
        @endforeach
      </tbody>
    </table></div>
    @endif
