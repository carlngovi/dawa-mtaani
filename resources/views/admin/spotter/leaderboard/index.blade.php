...
@section('title', 'Spotter Leaderboard — Dawa Mtaani')
...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="text-sm text-gray-400 mt-1">Spotter performance ranked by activations</p>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-700 text-gray-400 text-xs uppercase tracking-widest">
            ...
            <tbody class="divide-y divide-gray-700">
                @foreach($leaderboard as $row)
                    <tr class="{{ $loop->even ? 'bg-gray-900' : 'bg-gray-800' }}">
                        <td class="px-4 py-3 text-center font-bold {{ match($row['rank']) { 1 => 'text-yellow-400', 2 => 'text-gray-300', 3 => 'text-orange-400', default => 'text-gray-500' } }}">
                            {{ match($row['rank']) { 1 => '1st', 2 => '2nd', 3 => '3rd', default => $row['rank'].'th' } }}
                        </td>
                        <td class="px-4 py-3 text-white font-medium">{{ $row['name'] }}</td>
                        <td class="px-4 py-3 text-center {{ $row['submissions'] > 0 ? 'text-yellow-400' : 'text-gray-500' }}">
                            {{ $row['submissions'] }}
                        </td>
                        <td class="px-4 py-3 text-center text-lg font-bold {{ $row['activations'] > 0 ? 'text-green-400' : 'text-gray-500' }}">
                            {{ $row['activations'] }}
                        </td>
                    </tr>
                @endforeach
